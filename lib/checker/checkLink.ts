...
export async function checkLinkHealth(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      // If 5xx transient server error and we have retries left, retry
      ...
      if (httpStatus >= 500) {
        ...
        return {
          ...
          message: `HTTP ${httpStatus} Server Error on destination host (after ${attempt + 1} attempts)`,
          hasStateChanged: previousStatus !== newStatus,
          previousStatus,
        };
      }
  ...
  const responseTimeMs = Date.now() - startTime;
  const isTimeout = lastErr?.code === 'ECONNABORTED' || lastErr?.message?.includes('timeout') || lastErr?.code === 'ETIMEDOUT';
  const newStatus: LinkStatus = isTimeout ? 'warning' : 'broken';
  ...
  if (isTimeout) {
    return {
      ...
      message: `Connection timed out after ${maxRetries + 1} attempts (10,000ms limit)`,
      hasStateChanged: previousStatus !== newStatus,
      previousStatus,
    };
  }
  ...
  return {
    ...
    responseTimeMs,
    availabilityStatus: 'unavailable',
    errorType: lastErr?.message?.includes('ENOTFOUND') ? '404 Not Found' : 'Redirect Failed',
    message: lastErr?.message || 'Connection failed during HTTP resolution',
    hasStateChanged: previousStatus !== newStatus,
    previousStatus,
  };
}
